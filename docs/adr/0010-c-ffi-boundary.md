...
- **`unchecked` region** — the *only* place raw-pointer dereference, `extern`
  calls, and `assume` are allowed. All unsafety is named, contained, greppable.
...
Refinements crossing C→Clause are discharged by `assume` (trusted) or `dynamic`
(runtime-checked), per ADR-0001/Q4. Non-moving deterministic ARC (ADR-0009)
...
- Manual lifetime care inside `unchecked`; the type system does not track
  `Ptr<T>` validity.
- Auditing FFI safety = reading the `unchecked` regions and `assume` sites.
